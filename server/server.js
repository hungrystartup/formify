...
try {
    ...
    const requiredEnvVars = ['PORT', 'DATABASE_USER', 'DATABASE_NAME', 'DATABASE_HOST', 'DATABASE_PASSWORD', 'JWT_SECRET', 'FRONTEND_URL'];
    ...
    app.get("/api/total", verifyToken, async (req, res) => {
        try {
            const [rows] = await pool.execute("SELECT total_messages FROM users WHERE id = ?", [req.user.id]);
            if (rows.length === 0) return res.status(404).json({ error: "User not found" });
            res.json({ total: rows[0].total_messages });
        } catch (err) {
            console.error("Total fetch error:", err.message);
    ...
    app.get("/api/current", verifyToken, async (req, res) => {
        try {
            const [rows] = await pool.execute("SELECT current_message FROM users WHERE id = ?", [req.user.id]);
            if (rows.length === 0) return res.status(404).json({ error: "User not found" });
            res.json({ current: rows[0].current_message });
        } catch (err) {
            console.error("Current fetch error:", err.message);
            res.status(500).json({ error: "Server error" });
        }
    });
    ...
    app.get("/api/limit", verifyToken, async (req, res) => {
        try {
            ...
            console.error("Limit fetch error:", err.message);
            res.status(500).json({ error: "Server error" });
        }
    });
    ...
    app.post("/submit/:apikey", async (req, res) => {
        ...
        const cleanMessage = sanitizeHtml(message.trim(), { allowedTags: [], allowedAttributes: {} });
    ...
    app.get("/username", verifyToken, async (req, res) => {
        try {
            ...
            console.error("Username fetch error:", err.message);
            res.status(400).send({ success: false, error: err.message });
        }
    });
    ...
    app.get('/message-progress', verifyToken, async (req, res) => {
        ...
        try {
            ...
            console.error("Message progress error:", err.message);
