...
app.post('/signup', rateLimiter, async (req, res) => {
  ...
  try {
    if (!name || !email || !password) {
      return res
        .status(400)
        .send({ success: false, error: 'Fields cannot be empty' });
    }
    ...
    if (rows.length > 0) {
      return res
        .status(409)
        .send({ success: false, error: 'Email has already been used' });
    }
...
app.post('/v1/submit/:apikey', rateLimiter, async (req, res) => {
  ...
  if (!email || !message) {
    return res
      .status(400)
      .json({ success: false, error: 'Message content or email is required' });
  }
